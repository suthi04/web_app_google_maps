...
│   ├── preprocess.py      #  - ทำความสะอาด + ตัดคำไทย + แบ่งอนุประโยค (PyThaiNLP, มี fallback)
│   ├── clause.py          #  - แบ่งรีวิวเป็นอนุประโยคตามคำเชื่อมขัดแย้ง (แต่/แต่ว่า) ระดับ token
│   ├── negation.py        #  - จัดการคำปฏิเสธ (ไม่อร่อย) + แหล่งความจริงเรื่องขั้วของคำ
│   ├── sentiment.py       #  - จำแนกอารมณ์: WangchanBERTa / lexicon เมื่อ demo (+ ราย phrase ตามบริบท)
│   ├── lexicon.py         #  - ★ พจนานุกรม: คำนามหัวหมวด, descriptor, idiom, hint, synonym (เพิ่มคำที่นี่)
│   ├── aspect.py          #  - จับหมวด + ตัวแก้หมวดราย phrase 4 ชั้น (idiom→คำนาม→บริบท→hint)
│   ├── phrases/           #  - ★ สกัด "วลีความเห็น" (opinion phrases) — หัวใจของ Review Insight
│   │   ├── extract.py     #     · สกัดวลี: idiom/MWE → ไวยากรณ์เชิงพจนานุกรม (ไม่พึ่ง POS)
│   │   ├── quality.py     #     · กรองวลีขยะ (คำนามเดี่ยว/คำชวนเชียร์) + คุมการเติมคำนามหัว
│   │   ├── canonical.py   #     · ทำวลีให้เป็นรูปมาตรฐาน (ตัดคำขยาย/คำเชื่อม)
│   │   ├── synonyms.py    #     · รวมวลีความหมายเดียวกันแบบอนุรักษ์นิยม (ราคาไม่แพง=คุ้มค่า)
...
จุดที่มัก "ปรับบ่อย":
- **`core/lexicon.py`** → เพิ่มคำนามหัวหมวด/descriptor/idiom/synonym ให้สกัดวลีได้ครอบคลุมขึ้น
...
ระบบไม่ดึง "คำเดี่ยว" แต่สกัด **วลีความเห็นรูปมาตรฐาน** ผ่านลำดับขั้นที่เป็น
deterministic และอธิบายได้ (ไม่ใช่กล่องดำ):

```
รีวิว → แบ่งอนุประโยค (clause) → สกัดวลี (extract) → กรองคุณภาพ (quality)
      → ทำรูปมาตรฐาน (canonical) → รวมคำพ้อง (synonyms) → จัดหมวด 4 ชั้น (aspect)
      → จำแนกอารมณ์ตามบริบท (sentiment) → นับ/จัดอันดับ (aggregate) → แดชบอร์ด
```

แนวคิดสำคัญ:
- **ระดับอนุประโยค:** "อาหารอร่อย แต่บริการช้า" แยกเป็น 2 อนุประโยค เพื่อผูกอารมณ์เข้า
  หมวดให้ตรง (บริการไม่ถูกนับว่าบวกจากการชมอาหาร)
- **คำปฏิเสธ:** รวม "ไม่ + คำขั้ว" เป็นวลีเดียว (ไม่อร่อย, ราคาไม่แพง) กันความหมายกลับด้าน
- **สกัดเชิงพจนานุกรม (ไม่พึ่ง POS):** เราประเมิน POS tagging ของ PyThaiNLP แล้วพบว่า
  ติดป้ายคำแสดงความเห็นไทยผิดบ่อย (เช่น "อร่อย"→NOUN, "ดี"→ADV ในทุก corpus ที่ลอง)
  จึงเลือกไวยากรณ์เชิงพจนานุกรมที่กำหนดเอง (idiom/MWE + คู่ คำนาม+คำขยาย) ซึ่ง
  **อธิบายได้และคงเส้นคงวา** แทนตัวระบุชนิดคำที่ไม่น่าเชื่อถือ
- **รวมคำพ้องแบบอนุรักษ์นิยม:** รวมเฉพาะวลีที่สื่อความเดียวกันจริง (ราคาไม่แพง/ราคาดี/
  คุ้มค่า → "ราคาคุ้มค่า") ไม่รวมคำบรรยายที่ต่างความหมาย (อร่อย ≠ จัดจ้าน ≠ เข้มข้น)
- **จัดหมวด 4 ชั้น:** idiom/concept → คำนามหัว → บริบทอนุประโยค → คำขยายบ่งหมวด
  (รองรับวลีไร้คำนาม เช่น ติดริมน้ำ, เย็นสบาย, คึกคัก)
- **แยกการสกัดออกจากการตัดสินอารมณ์:** อารมณ์ของแต่ละวลีตัดสิน "ตามบริบทอนุประโยค"
  (คนเยอะ อาจบวกหรือลบขึ้นกับบริบท) ไม่ใช่จากตัววลีลำพัง

### รันชุดทดสอบ
```bash
python -m unittest discover -s tests      # 108 เทสต์ (ไม่ต้องติดตั้ง pytest)
```

### ข้อจำกัด (Limitations)
- แบ่งอนุประโยคเฉพาะคำเชื่อมขัดแย้งกลุ่ม "แต่" (อนุรักษ์นิยม กันแบ่งผิด)
- ความครอบคลุมของวลีจำกัดด้วยขนาดพจนานุกรม (คำที่ไม่อยู่ใน lexicon อาจหลุด)
- ขอบเขตคำปฏิเสธมองแค่คำขั้วที่ติดกัน 1 คำ
- คุณภาพวลี/การจัดหมวด ตรวจเชิงคุณภาพด้วยตัวอย่างจริง (ยังไม่มีชุด gold ราย phrase)
...
- [x] สกัด **วลีความเห็น (opinion phrases)** ด้วย POS-pattern chunking เชิงพจนานุกรม (เช่น อาหารอร่อย, ราคาไม่แพง, รอนาน, ติดริมน้ำ)
