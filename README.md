...
หากบริการดึงรีวิวหรือโมเดลขัดข้อง ให้กด **ดูข้อมูลตัวอย่าง** ในหน้าข้อผิดพลาด
หรือกล่องสถานะงาน เพื่อเปิด `/sample` ได้ทันที ทั้งมุมผู้บริโภคและผู้ประกอบการ
หน้านี้อ่านผลสำเร็จรูปจาก `data/sample_analysis.json` ซึ่งสร้างด้วย Lexicon + Rule-based
จากรีวิวเดิมใน `data/sample_reviews.json` โดยไม่เรียก Apify/Gemini/โมเดล ไม่เข้าคิวงาน
และไม่บันทึกทับประวัติของผู้ใช้ มีป้ายระบุว่าเป็นข้อมูลตัวอย่างเสมอ
เว็บเซิร์ฟเวอร์ยังต้องทำงานอยู่จึงจะเปิดหน้านี้ได้
