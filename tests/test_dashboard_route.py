...
class TestAudienceDashboard(unittest.TestCase):
    ...
    def test_renders_consumer_and_operator_persona_tabs(self):
        ...
        self.assertIn("สรุปสำหรับผู้ประกอบการ", html)
        self.assertIn("Strategic Roadmap", html)
        self.assertIn("Tactical Playbook", html)
        self.assertIn("รอบถัดไป ต้องเช็กอะไร", html)
        self.assertNotIn("Top mentions", html)
        self.assertIn('id="consumerReviews"', html)
        self.assertNotIn('id="consumerReviewPagination"', html)
        self.assertNotIn('data-page-action="next"', html)
    ...
    def test_operator_view_uses_one_ranked_plan_without_duplicate_sections(self):
        ...
        self.assertIn('id="operatorPlaybookTitle"', html)
        self.assertIn("แก้จุดสะดุดของลูกค้า", html)
        self.assertIn("ขยายสิ่งที่ลูกค้ารัก", html)
        ...
        self.assertIn('id="operatorResultsTitle"', html)
        self.assertIn('class="panel result-panel operator-result-panel"', html)
        self.assertIn('class="operator-result-counts"', html)
        self.assertIn('class="operator-review-meta"', html)
        self.assertIn('id="operator-review-R001"', html)
        self.assertIn('class="operator-task-detail"', html)
        self.assertIn('class="operator-deep-dive"', html)
        self.assertNotIn('id="operatorReviewPagination"', html)
        self.assertIn('data-review-id="R001"', html)
        self.assertIn("js/dashboard.js?v=20260813-reviews1", html)
        self.assertIn("กรองอารมณ์", html)
        self.assertNotIn('class="metrics"', html)
        self.assertNotIn('class="panel sentiment-panel"', html)
        evidence_overview = html.index('id="operatorEvidenceOverviewTitle"')
        roadmap = html.index("ลำดับงานที่ทีมควรเดินต่อ")
        self.assertLess(evidence_overview, roadmap)
