...
class TestAudienceDashboard(unittest.TestCase):
    ...
    def test_operator_view_uses_one_ranked_plan_without_duplicate_sections(self):
        ...
        self.assertIn('id="operatorRoadmapTitle"', html)
        self.assertIn('id="operatorEvidenceTitle"', html)
        self.assertIn("Evidence Studio", html)
        self.assertIn("รีวิวและคำที่ลูกค้าพูดจริง", html)
        self.assertNotIn('class="metrics"', html)
        self.assertNotIn('class="lower"', html)
    ...
    def test_long_analysis_result_is_scrollable_without_disclosure(self):
        ...
        html = _get_dashboard(payload)
        self.assertIn('class="operator-result-panel result-panel"', html)
