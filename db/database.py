...
def update_narrative(aid: int, narrative: dict) -> bool:
    """อัปเดตเฉพาะฟิลด์ narrative ใน payload (ใช้ตอน 'สร้างเนื้อหา AI ใหม่')
    โดยไม่แตะผลวิเคราะห์ส่วนอื่น — ไม่ต้องดึงรีวิว/รัน pipeline ใหม่."""
    with _conn() as c:
        row = c.execute(
            "SELECT payload FROM analysis WHERE id = ?", (aid,)
        ).fetchone()
        if not row:
            return False
        data = json.loads(row["payload"])
